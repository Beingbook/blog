...
description: React는 어떻게 컴포넌트 상태를 관리할까요?
createdAt: 2019-04-16
status: published
---

React Hook 정식 릴리즈 한 뒤 1분기가 지나갑니다.
아주 편리하고 매혹적인 기능입니다.
...
성능에 치명적인 부분에서 사용하면 됩니다.

## 일괄 상태 갱신

React가 컴포넌트를 렌더링 하는 동안 `setState`를 호출하더라도 상태를 즉시 바꾸진 않습니다.
...
클래스 컴포넌트의 `this.setState`도 큐를 이용하지만
[`this.setState`는 `react-dom`, `react-native`같은 렌더러들이 주입해주는 큐를 사용합니다.](https://github.com/facebook/react/blob/d584fcdc6e10b9f4d4e9eec775731e7ffc0fb501/packages/react/src/ReactBaseClasses.js#L21)
...
단순한 배열 대신에 연결 리스트를 사용하여 구현되어 있습니다.
...
4. React 내부에는 상태 관리를 위해 연결 리스트를 사용합니다.
